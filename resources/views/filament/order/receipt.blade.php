...
<div x-data="{
    ...
    id="receipt-content">
    <h2 class="mb-4 text-xl font-semibold text-gray-900 dark:text-white">Order #{{ $id }}</h2>
                        ...
                        @php
                            ...
                            if ($item->orderable) {
                                if (get_class($item->orderable) === 'App\Models\Food' && $item->orderable->foodDetail) {
                                    $itemName =
                                        $item->orderable->foodDetail->name .
                                        ' - ' .
                                        $item->orderable->servingType->name .
                                        ' (' .
                                        $item->quantity .
                                        ' pax)';
    ...
    <table class="w-full text-left border-collapse table-auto">
        ...
        <tbody>
            ...
            @endphp
            @if (count($payments) <= 0)
                <td class="self-center w-full p-2 text-gray-800 dark:text-gray-200">No payments
                    yet.</td>
            @endif
    ...
    <div class="mt-6 flex justify-center">
        <button x-on:click="printDiv()"
            class="px-6 py-3 text-sm font-medium text-dark dark:text-white bg-blue-500 rounded-lg shadow-md transition-all duration-200 hover:bg-blue-600 focus:ring-2 focus:ring-blue-300 focus:outline-none dark:bg-blue-700 dark:hover:bg-blue-800 dark:focus:ring-blue-500">
            Print Receipt
        </button>
    </div>
</div>
