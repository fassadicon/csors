<div class="grid grid-cols-2 gap-4">
    ...
    <div>
        <x-mary-header title="{{ $utility->name }}" class="!my-2">
        </x-mary-header>

        <div class="mt-4">
            <x-mary-badge value="{{ 'PHP ' . $utility->price }}" class="text-white badge-success" />
        ...
        <form wire:submit="addToCart">
            <div class="flex mt-4 space-x-4">
                <div class="w-1/3">
                    ...
                    <x-text-input wire:model.live="quantity" id="quantity" class="block w-full mt-1" type="number"
                        name="quantity" required autocomplete="quantity" />
                    <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
                </div>
                <div class="w-2/3">
                    <x-input-label for="price" :value="__('Price')" />
                    <x-text-input wire:model="price" id="price" class="block w-full mt-1" type="number" required
                        readonly autofocus prefix="PHP" autocomplete="price" />
