<div class="p-4 bg-jt-white rounded-md">
    ...
    <x-mary-header title="Customer Information"
        class="!my-2" subtitle="Customer: {{ $order->user->name }}"
        separator />
    ...
    <div class="flex flex-col gap-y-2 p-4 md:p-0 w-[90%] md:w-[45%] mt-4">
        <div class="flex gap-x-2">
            ...
            <x-primary-button class="w-full bg-jt-primary-dark flex !justify-center !text-center" wire:click='payPartial'>{{ __('Pay Partial') }}</x-primary-button>
            <x-primary-button class="w-full btn-primary flex !justify-center" wire:click='payFull'>{{ __('Pay Full') }}</x-primary-button>
            @elseif ($order->payment_status->value == 'partial')
            <x-primary-button class="w-full btn-primary flex !justify-center" wire:click='payRemaining'>{{ __('Pay Remaining Balance') }}</x-primary-button>
            @endif
        </div>
        {{-- CANCEL  --}}
        @unless ($order->cancellationRequest)
            @if ($canRequestCancellation)
                <x-danger-button class="flex justify-center bg-slate-900" wire:click='cancel'>{{ __('Request to Cancel') }}</x-danger-button>
            @endif
        @endunless
