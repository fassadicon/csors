        ...
        <div class="flex flex-col justify-end mt-4 gap-y-4">
            <div class="flex flex-row justify-end gap-x-2">
                <x-mary-badge value="Payment: {{ $order->payment_status->getLabel() }}"
                    class="{{ $order->payment_status->getMaryColor() }}" />
                <x-mary-badge value="Order: {{ $order->order_status->getLabel() }}"
                    class="{{ $order->order_status->getMaryColor() }}" />
            </div>
    ...
    <div class="">
        <div class="flex flex-col items-start justify-between w-full p-4 md:flex-row gap-y-4">
            <div class="space-y-2">
                ...
                <p class="text-gray-900 dark:text-gray-100">
                    Start: {{ $order->start->format('M j, Y - g:ia') }} - End: {{ $order->end->format('M j, Y - g:ia') }}
            ...
            <hr class="block my-4 md:my-0 md:hidden">
            <div class="flex justify-evenly flex-col gap-y-2 p-4 md:p-0 w-[90%] md:w-[45%] ">
                ...
                @unless ($order->cancellationRequest)
                    @if ($canRequestCancellation)
                        <x-danger-button class="flex justify-center bg-red-700"
                            wire:click='cancel'>{{ __('Request to Cancel') }}
                        </x-danger-button>
                    @endif
                @endunless
                {{-- TEMPORARY  --}}
                @if ($canPay && $order->payment_status->value == 'pending')
